...
void ghc::filesystem::to_json(json11::Json& json, path const& path) {
    json = path.string();
}

void ghc::filesystem::from_json(json11::Json const& json, path& path) {
    path = json.string_value();
}
...
Result<json11::Json> utils::file::readJson(ghc::filesystem::path const& path) {

    auto str = utils::file::readString(path);

    if (str) {
        try {
            return Ok(json11::Json::try_parse(str.value()));
